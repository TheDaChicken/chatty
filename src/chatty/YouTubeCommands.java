...
/**
 * Allows commands like the Twitch Chat to be sent in chatty since YouTube doesn't have commands in their chat.
 *
 * @author TheDaChicken/tduva
 */
public class YouTubeCommands {

    private static final Logger LOGGER = Logger.getLogger(YouTubeCommands.class.getName());
     ...
     */
    private static final Set<String> NO_PARAMETER_COMMANDS = new HashSet<>(Arrays.asList("followersoff", "subscribers", "subscribersoff", "slowoff", "emoteonly",
            "emoteonlyoff", "r9kbeta", "r9kbetaoff"));

    private YouTubeConnection c;

    public YouTubeCommands(YouTubeConnection c) {
        this.c = c;
    }
    ...
    public void ban(String channel, String channel_id, String reason) {
        if (onChannel(channel, true)) {
            c.ban(channel, channel_id, "Trying to ban "+channel_id+"..");
        }
    }

    private void commandUnban(String channel, String parameter) {
        if (StringUtil.isNullOrEmpty(parameter)) {
            sendMessage(channel, "/unban", "Trying to unban..");
            return;
        }
        String[] parts = parameter.split(" ", 2);
        String nick = parts[0];
    ...
    public void unban(String channel, String channel_id) {
        if (onChannel(channel, true)) {
            c.unban(channel, channel_id, "Trying to unban "+channel_id+"..");
        }
    }
    ...
    public void timeout(String channel, String msgId, String name, String time, String timeLabel, String reason) {
        if (onChannel(channel, true)) {
            c.timeout(channel, msgId, name, time, timeLabel, reason, "Trying to timeout "+name+" ("+timeLabel+")");
        }
    }
