...
import chatty.gui.components.menus.CommandMenuItemsN;
...
public class ButtonsCustom {
    ...
    public ButtonsCustom(UserInfo owner, ActionListener listener) {
        this.owner = owner;
    ...
    private void add(String setting) {
        ...
        List<CommandMenuItem> items = CommandMenuItemsN.parse(setting);
        for (CommandMenuItem item : items) {
            ...
            if (item.getCommand() != null && item.getCommand().getCommandName() != null) {
                ...
                if (commandName.equalsIgnoreCase("modunmod")) {
                    ...
                    // Needs to be set to one odf the two valid states so it can
    ...
    public JPanel getRow(String row) {
        ...
        if (row.equals(CommandMenuItemsN.CUSTOM_COMMANDS_SUBMENU)) {
            row = "b1";
        }
